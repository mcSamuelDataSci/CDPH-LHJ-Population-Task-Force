...
This RMarkdown file creates a dataset with age-race/ethnicity-sex population estimates and projections for:
...
1.  outputs/intermediate/census-city-ars-historical.RDS & outputs/intermediate/census-city-ars-current.RDS

    1.  City-level Sex-Age-Race/Ethnicity population estimates pulled from the 2000, 2010, and 2020 Decennial.

2.  outputs/intermediate/census-county-ars-historical.RDS & outputs/intermediate/census-county-ars-current.RDS

    1.  County-level Sex-Age-Race/Ethnicity population estimates pulled from the 2000, 2010, and 2020 Decennial.

3.  outputs/intermediate/dof-p3-processed-historical.RDS & outputs/intermediate/dof-p3-processed-current.RDS

    1.  Processed Sex-Age-Race/Ethnicity population estimates and projections from 2000+

4.  outputs/intermediate/lhj-population-total.RDS

    1.  Total July LHJ population estimates and projections

Final output is saved as csv and RDS file in outputs/final/:

1.  lhj-pop-ars-2000-2030
...
Note: The total county population estimates in **DOF-P3** already match the July county estimates in **DOF-E6** for 2000-2019.

```{r}
# Adjusting county ARS (p3) to dof e6
countyARS <- p3 %>% 
...
-   decennial 2020 city:county proportions are multiplied to DOF P3 2020+ data

```{r}

joinCityCounty <- function(myYears, mySource = NULL) {
  
  decennialYear <- min(myYears)
  # myYears <- myYear:(myYear+9)
  
  tCity <- censusCityCountyARS %>% filter(year == decennialYear) %>% select(-year)
...
# 2000+ data using DOF's intercensal estimates (2000-2019) and most recent P3 (2020+) 

maxYear <- max(countyARS$year)

cityARS_final <- bind_rows(
  joinCityCounty(2000:2009),
  joinCityCounty(2010:2019),
  joinCityCounty(2020:maxYear)
) %>% 
  mutate(population = cityCountyProp * population)%>%
...
Note: Negative population estimates are calculated for 26 sex-age-race-health department stratas. These all occur in Multi-Race and AI/AN 95+ year olds whose populations are very small. These negative values are set to 0.
...
ggplot(checkTotals, aes(x = year, y = population)) +
  geom_line() +
  facet_wrap(vars(race_eth), scales = "free_y") +
  scale_x_continuous(minor_breaks = 2000:maxYear, breaks = 2000:maxYear, labels = 2000:maxYear) +
  theme(axis.text.x = element_text(angle = 90, hjust = 1, vjust = 0.5))

# Compare to prior file
lhjARS_old <- readRDS(paste0(outputDir, "lhj-pop-ars.RDS")) %>% 
  rename(old = population)

comp <- full_join(
  lhjARS_old, 
  rename(lhjARS, new = population)
) %>% 
  mutate(diff = new - old, 
         pdiff = diff / old)
...
comp %>% 
  ...
  group_by(year, county_lhj) %>% 
  summarise(old = sum(old), 
            new = sum(new)) %>% 
  ungroup() %>% 
  mutate(diff = new - old) %>% 
  View()
...
lhjARS <- lhjARS %>% 
  mutate(county_indicator = ifelse(county_lhj %in% c("Berkeley", "Long Beach", "Pasadena", "Alameda HD", "Los Angeles HD"), 0, 1), 
         lhj_indicator = ifelse(county_lhj %in% c("Alameda", "Los Angeles"), 0, 1)
         )

# Save data -------------------------------------

saveRDS(lhjARS, paste0(outputDir, "lhj-pop-ars.RDS"))
write_csv(lhjARS, paste0(outputDir, "lhj-pop-ars.csv"))
