...
Note: This Rmarkdown only needs to be executed once to produce the historical population datasets. Once produced, this file never needs to be ran again **unless** any of the historical data files (listed further below) are updated.
...
1.  Department of Finance (DOF) Annual Intercensal Population Estimates by Race/Ethnicity with Age and Gender Detail
    ...
    2.  Location: *inputs/DOF/Annual Intercensal/2000-2010/Intercensal_2000-2010_DBInput.csv*
...
4.  Department of Finance - E-6 Population Estimates and Components of Change by County
    ...
    1.  Excel files of these data were downloaded from [here](https://dof.ca.gov/forecasting/demographics/estimates/), and are stored in *inputs/DOF/E6/Raw/*. The raw data were then manually formatted for easier processing and stored into *inputs/DOF/E6/Clean/.*

    2.  Contains July 1st county-level total estimates from 1947-2009

Note: The data structure and formatting for each of DOF's raw E-4 and E-6 files are largely inconsistent. Therefore, these files were first manually transformed into a standard format.
...
p3_historical <- read_csv("inputs/DOF/Annual Intercensal/2000-2010/Intercensal_2000-2010_DBInput.csv") %>% 
...
# Save data as both csv and RDS into into outputs/intermediate/ =======
...
decLink2000 <- read_excel("inputs/Standards/decennialLink.xlsx", sheet = "2000 ARS")
decLink2010 <- read_excel("inputs/Standards/decennialLink.xlsx", sheet = "2010 ARS")
...
e6_1947_69 <- read_excel("inputs/DOF/E6/Clean/E6-1947-1969.xlsx", col_names = c("county", 1947:1969), skip = 1) %>% 
