...
1.  Department of Finance (DOF) Annual Intercensal Population Estimates by Race/Ethnicity with Age and Gender Detail
    ...
    1.  Location: *inputs/DOF/Annual Intercensal/2010-2020/intercensals_2010_2020.csv*

    2.  Downloaded from [here](https://dof.ca.gov/forecasting/demographics/estimates/annual-intercensal-population-estimates-by-race-ethnicity-with-age-and-gender-detail-2010-2020/)

    3.  2010-2019 population estimates are used.

    4.  Contains sex by age by race/ethnicity population estimates at the county-level.

2.  Department of Finance (DOF) P3 Complete State and County Projections Dataset

    1.  Location: *inputs/DOF/P3/P3_Complete.csv*

    2.  Downloaded from [here](https://dof.ca.gov/forecasting/demographics/projections/)

    3.  2020+ population estimates and projections are used.
...
4.  Department of Finance - E-4 Population Estimates for Cities, Counties, and the State
    ...
    1.  Excel files of these data were downloaded from [here](https://dof.ca.gov/forecasting/demographics/estimates/), and are stored in *inputs/DOF/E4/Raw/*. The raw data were then manually formatted for easier processing and stored into *inputs/DOF/E4/Clean/*.

    2.  Contains January 1st county- and city-level total estimates from 2011+

5.  Department of Finance - E-6 Population Estimates and Components of Change by County

    1.  Excel files of these data were downloaded from [here](https://dof.ca.gov/forecasting/demographics/estimates/), and are stored in *inputs/DOF/E6/Raw/*. The raw data were then manually formatted for easier processing stored into *inputs/DOF/E6/Clean/.*

    2.  Contains July 1st county-level total estimates from 2010+

Note: The data structure and formatting between each of DOF's raw E-4 and E-6 files are largely inconsistent. Therefore, these files were first manually transformed into a standard format.

## Data File Outputs:

The processed datasets are saved as csv and RDS files into *outputs/intermediate/*:
...
Note: The final product (LHJ-level sex-age-race/ethnicity population estimates - created in *3-Create-LHJ-Population-ARS.Rmd*) spans from 2000 to the most recent year of data available in DOF-E6 (**currentYear** object below) + 5 years.
...
# Set current year
currentYear <- 2025 # Jan E6 - most recent year available
currentYear_Jan <- 2025 # Jan E4 - most recent year available

# Projection years: E6 max year + 5
projectionYear <- currentYear+5
...
outputDir <- "outputs/intermediate/"

```

## Process and Save Data

### Department of Finance's Annual Intercensal and P3 Complete State and County Projections Dataset

```{r}

# County relationship file: Used for linking DOF's county codes to county names. =======
countyLink <- read_xlsx("inputs/Standards/countyLink.xlsx") %>% 
...
raceLink <- read_xlsx("inputs/Standards/raceLink.xlsx") %>% 
...
if (F) {
  ...
  p3_10_60 <- read_csv("inputs/DOF/P3/Archive/P3_Complete_2010-2060.csv") %>%
  ...
  p3_20_ <- read_csv("inputs/DOF/P3/Archive/P3_Complete_2020-2060.csv") %>% 
...
intercensal_10_19 <- read_csv("inputs/DOF/Annual Intercensal/2010-2020/intercensals_2010_2020.csv") %>% 
  filter(month == 7) %>% 
  left_join(countyLink, by = "fips") %>% 
  left_join(raceLink) %>% 
  mutate(sex = str_to_title(sex)) %>% 
...
# DOF P3 Vintage 2025 (2025.09.30 version)
p3_20_ <- read_csv("inputs/DOF/P3/P3_Complete.csv") %>% 
  filter(year %in% 2020:projectionYear) %>% 
  left_join(countyLink, by = "fips") %>% 
  left_join(raceLink) %>%
  mutate(sex = str_to_title(sex)) %>% 
  select(county, year, sex, raceEth, age = agerc, population = perwt)
...
.ckey   <- read_file("inputs/Standards/census.api.key.txt")
...
decLink2020 <- read_excel("inputs/Standards/decennialLink.xlsx", sheet = "2020 ARS")
...
process_e4 <- function(years) { 
  ...
  fileName <- ifelse(2021 %in% years, 
                     "inputs/DOF/E4/Clean/E4-2021-cy.xlsx",
                     paste0("inputs/DOF/E4/Clean/E4-", min(years), "-", max(years), ".xlsx"))
...
process_e6 <- function(years) {
  ...
  if (F) {
    years <- 2020:currentYear
  }
  
  fileName <- ifelse(2021 %in% years, 
                     "inputs/DOF/E6/Clean/E6-2020-cy.xlsx",
                     paste0("inputs/DOF/E6/Clean/E6-", min(years), "-", max(years), ".xlsx"))
