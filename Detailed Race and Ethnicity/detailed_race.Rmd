...
flextable(iTable) %>%
  ...
  footnote(i = 18, j = 2,
           ...
           ref_symbols = c("**"),
           part = "body") %>%
  fontsize(size = 12, part = "all") %>% padding(padding=2, part="all") %>%
...
-   American Community Survey (ACS) Public Use Microdata Sample (PUMS)
    ...
    -   [American Community Survey (ACS) Public Use Microdata Sample (PUMS) homepage](https://www.census.gov/programs-surveys/acs/microdata.html)
...
-   "Regular" American Community Survey (ACS)
    ...
    -   [American Community Survey (ACS) homepage](https://www.census.gov/programs-surveys/acs)
...
-   The map below shows an overlay of counties and PUMAs (2020 boundaries). An excel file linking 2020 PUMA boundaries to counties will be included in this "repository" soon.

```{r, echo = F}
# Maybe add hover-over or county labels

library(tigris)
pumaB   <- pumas(state = "CA", class = "sf", cb = TRUE, year = 2020, progress_bar = FALSE)
countyB <- counties(state = "CA", class = "sf", cb = TRUE, year = 2020, progress_bar = FALSE )
...
Due to the long download time via tidycensus, data were instead downloaded manually from ACS PUMS. 
...
stateDataYear <- 2022 # ACS 2022 5Y Survey

ageGroups <- c('0 - 1', '1 - 4', '5 - 14', '15 - 24', '25 - 34', 
               '35 - 44', '45 - 54', '55 - 64', '65 - 74', '75 - 84', '85+') 

# Read in info files ==================================================================
pumsCol <- paste0("pums_raceName_", stateDataYear)

detailedRaceLink <- read_excel("detailedRaceLinkShort.xlsx", sheet = "detailedRaceLink") |>
  select(detailedRace = {{ pumsCol}}, raceGroup) |>
  filter(!is.na(detailedRace))


# Read in ACS PUMS data ===========================================================

rawPop_race_age <- read_csv(paste0('data in/raw/Detailed_Race_by_Age_Pop_PUMS', stateDataYear, '.csv'), skip = 4)
...
# Clean data frame =================================================================================
pop_race_age <- rawPop_race_age[c(-1, -2), ] |>
  select(-Total) |>
  rename(detailedRace = 1) |>
  mutate(detailedRace = sub(".*-> Total -> Total California", "", detailedRace)) |>
  mutate(detailedRace = sub(".* -> Total ", "", detailedRace)) |>
  mutate(ageGroup = ifelse(!(detailedRace %in% ageGroups), 
                           "Total", detailedRace)) 


# Create Race, Non-Hispanic Population Dataset ========================================================

nh_pop <- pop_race_age |>
  select(detailedRace, ageGroup, population = "Not Spanish/Hispanic/Latino")

nh_race_groups <- detailedRaceLink |>
  filter(!raceGroup %in% c("Latino", "Total")) |>
  dplyr::distinct(detailedRace) |>
  dplyr::pull(detailedRace)


## Check ======================================================

# Are Detailed Race/Ethnicity names are identical between the PUMS data and detailedRaceLink?
# This can happen where they are not identical. For example, detailed RE categories were removed or renamed in ACS 2022 compared to 2019
diff1 <- setdiff(nh_pop$detailedRace, nh_race_groups)
diff1 <- diff1[!diff1 %in% ageGroups]
diff2 <- setdiff(nh_race_groups, nh_pop$detailedRace)

if (length(diff1) + length(diff2) > 0) {
  stop("Detailed Race/Ethnicity names are not identical between the PUMS data and detailedRaceLink. Investigate.")
} 
...
nh_pop_list <- lapply(nh_race_groups, function(x) {
  
  race_index <- which(nh_pop$detailedRace == x)
  age_index <- race_index + n_agegroups
  
  nh_pop[race_index:age_index, ] |>
    mutate(detailedRace = x)
  
  
})

nh_pop_final <- dplyr::bind_rows(nh_pop_list)

# Create Hispanic Population Dataset ===============================

hisp_pop <- pop_race_age |>
  select(-detailedRace, -`Not Spanish/Hispanic/Latino`) |>
  pivot_longer(-ageGroup, values_to = 'population', names_to = 'detailedRace') |>
  group_by(detailedRace, ageGroup) |>
  summarise(population = sum(population)) |>
  dplyr::ungroup()

# Bind Race, NH & Hispanic population datasets ======================

pop_race_age_final <- dplyr::bind_rows(nh_pop_final, hisp_pop) |>
  dplyr::left_join(detailedRaceLink, by = "detailedRace") %>% 
  mutate(ageGroup = ifelse(ageGroup == '0 - 1', '0', ageGroup))

# Data Validation Checks =================================

if (F) {
  colSums(is.na(pop_race_age_final))
  table(pop_race_age_final$ageGroup, useNA = "ifany")
  table(pop_race_age_final$detailedRace, useNA = "ifany")
  table(pop_race_age_final$raceGroup, useNA = "ifany")
}

# Save Population data ========================================================
write_csv(pop_race_age_final, 'data out/ca_pop_detailed_RE_age_pums.csv')
...
library(readxl)
acsDetailedAsian <- read_xlsx("data in/detailedAsian_ACS_link.xlsx", sheet = "y2022")
acsDetailedLatino <- read_xlsx("data in/detailedLatino_ACS_link.xlsx", sheet = "y2010-2022")
acsDetailedNHPI <- read_xlsx("data in/detailedNHPI_ACS_link.xlsx", sheet = "y2022")


ourGet <- function(ourVariable = acsDetailedAsian) {
  get_acs(geography = "county", state = 06, year = 2022, survey = "acs5", moe_level = 90, variables = ourVariable$acsID) %>% 
